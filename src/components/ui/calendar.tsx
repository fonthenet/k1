...
function CalendarDropdown({
  ...
  return (
    <>
      {/* Bridge DayPicker's native change contract without exposing a browser menu. */}
      <select
        hidden
        aria-hidden="true"
        tabIndex={-1}
      ...
      <Select
        value={String(value)}
        disabled={disabled}
        dir={dayPickerProps.dir === "rtl" ? "rtl" : "ltr"}
        onValueChange={(next) => {
      ...
      >
        <SelectTrigger
          size="sm"
          aria-label={label}
          className="h-(--cell-size) border-0 px-1.5 shadow-none"
        >
          <SelectValue />
        </SelectTrigger>
