 ...
 */
export function DeleteStructureButton({
  structureId,
  structureName: name,
  classCount,
  childCount,
  ...
  // not have to thread the building's structure list into every card.
            ...
            <div className="grid gap-3 rounded-lg border border-border bg-muted/30 p-3">
              <div className="grid gap-1.5">
                ...
                <Select value={toStructureId} onValueChange={chooseStructure}>
                  <SelectTrigger id="move-all-structure" className="w-full bg-background">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {targets.structures.map((s) => (
                      <SelectItem key={s.id} value={s.id}>
                        {/* The structure's own colour, the one signal, as the
                            switcher and the comms picker give it. */}
                        <span
              ...
              {targetClasses.length > 0 && (
                <div className="grid gap-1.5">
                  ...
                  <Select value={toClassId} onValueChange={setToClassId}>
                    <SelectTrigger id="move-all-class" className="w-full bg-background">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <p className="text-xs text-muted-foreground">{t("structures.moveAll.hint")}</p>
              <Button
                variant="outline"
                onClick={moveAll}
